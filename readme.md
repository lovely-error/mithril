...
5. Control flow through address manipulation and stores/loads from ip register
...
9.  Simplified cache coherence (only E of MESI) on L2 (Only two operations that atomically operate on extmem) (scpad is coherent because only one core writes to it, and no speculation ever occurs) (memory subsystem is only responsible for tracking operations on extmem)
      1. One is `try load exclusive with flag` which returns a value in one register and flag in another indicating if a core has a cache line locked. If flag is false, the return value is not most recent . If true, it is most recent.
...
1. Code with no inherent ILP may be bloated with nops
   ...
   2. Fixable with "parity check" on one bit in instructions. `[I1:1, I2:0, I3:0, I4:0]` split this into two packs by masking on the bit. And execute each resultant consequently (suboptimal solution)
   3. Maybe, real code dont suffer from this much (no longer relevant)
2. Loops have to not be sparse and NPUP does not solve this.
...
4. Pack width tied to impl can cause binary-level portability issues
   1. Code compatibility should not be a cpu design concern (its an OS (or higher) layer concern!)
